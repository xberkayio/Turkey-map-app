  ...
  return (
    <div className='mt-4'>
      ...
      {zoomedCity && (
        ...
        onMouseOver={(e) => e.currentTarget.style.backgroundColor = "#40b5ae"}
        ...
        {showSideMenu && selectedInstitution && (
          <div className='side-menu' style={{
            ...
            <button 
              ...
              onMouseOver={(e) => e.currentTarget.style.backgroundColor = "#ff5a4c"}
              ...
              {selectedInstitution.website && (
                <div className="mb-3 text-center">
                  <a 
                    ...
                    onMouseOver={(e) => {
                      e.currentTarget.style.backgroundColor = '#6bc093';
                      e.currentTarget.style.transform = 'translateY(-2px)';
                      e.currentTarget.style.boxShadow = '0 5px 10px rgba(0,0,0,0.15)';
                    }}
                    onMouseOut={(e) => {
                      e.currentTarget.style.backgroundColor = '#88d8b0';
        ...
        <div className="col-md-12">
          <div className="card shadow-sm">
            <div className="card-body">
              <h5 className="card-title text-primary">Genel İstatistikler</h5>
              <div className="row mt-3">
                <div className="col-md-3 text-center mb-3">
                  <div className="border rounded p-3 h-100 d-flex flex-column justify-content-center">
                    <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#ff6f61' }}>
                      {Object.keys(sehirRusya).reduce((total, city) => total + sehirRusya[city].length, 0)}
                    </div>
                    <div className="text-muted">Toplam Rus Kurumu</div>
                  </div>
                </div>
                <div className="col-md-3 text-center mb-3">
                  <div className="border rounded p-3 h-100 d-flex flex-column justify-content-center">
                    <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#88d8b0' }}>
                      {Object.keys(sehirRusya).length}
                    </div>
                    <div className="text-muted">Faaliyet Gösterilen İl</div>
                  </div>
                </div>
                <div className="col-md-3 text-center mb-3">
                  <div className="border rounded p-3 h-100 d-flex flex-column justify-content-center">
                    <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#ffcc29' }}>
                      {Object.values(sehirRusya).flat().filter(item => item.type === 'Kültür').length}
                    </div>
                    <div className="text-muted">Kültür Kurumu</div>
                  </div>
                </div>
                <div className="col-md-3 text-center mb-3">
                  <div className="border rounded p-3 h-100 d-flex flex-column justify-content-center">
                    <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#6b5b95' }}>
                      {Object.values(sehirRusya).flat().filter(item => item.type === 'Konsolosluk' || item.type === 'Büyükelçilik').length}
                    </div>
                    <div className="text-muted">Diplomatik Temsilcilik</div>
                  </div>
                </div>
              </div>
            </div>
      ...
      {/* En çok Rus kurumu bulunan şehirler */}
      <div className="row mt-4">
        <div className="col-md-6">
          <div className="card shadow-sm">
            <div className="card-body">
              <h5 className="card-title text-primary">En Çok Rus Kurumu Bulunan Şehirler</h5>
              <ul className="list-group list-group-flush mt-3">
                {Object.keys(rusKurumSayilari)
                  .sort((a, b) => rusKurumSayilari[b] - rusKurumSayilari[a])
                  .slice(0, 3)
                  .map(cityCode => (
                    <li key={cityCode} className="list-group-item d-flex justify-content-between align-items-center">
                      <span>{document.getElementById(cityCode)?.getAttribute('title') || 'Şehir'}</span>
                      <span className="badge bg-primary rounded-pill">{rusKurumSayilari[cityCode]} kurum</span>
                    </li>
                  ))
                }
              </ul>
        ...
        {/* Kurum türleri dağılımı */}
        <div className="col-md-6">
          <div className="card shadow-sm">
            <div className="card-body">
              <h5 className="card-title text-primary">Kurum Türleri Dağılımı</h5>
              <div className="mt-3">
                {['Büyükelçilik', 'Konsolosluk', 'Ticaret', 'Enerji', 'Kültür'].map(type => {
                  const count = Object.values(sehirRusya).flat().filter(item => item.type === type).length;
                  const percentage = Math.round((count / Object.values(sehirRusya).flat().length) * 100);
                  
                  return (
                    <div key={type} className="mb-2">
                      <div className="d-flex justify-content-between mb-1">
                        <span>{type}</span>
                        <span>{count} kurum ({percentage}%)</span>
                      </div>
      ...
      {/* Footer */}
      <footer className="mt-4 mb-5 text-center text-muted">
        <p className="small">© 2025 Türkiye'deki Rus Kurumları Haritası</p>
        <p className="small">Bu harita, Türkiye'deki Rus diplomatik, ticari ve kültürel kurumları hakkında bilgilendirme amaçlıdır.</p>
      </footer>
